...
def create_exit_intent(context):
    i = Intent(context, PythonActivity)
    i.putExtra(AndroidString("exit"), AndroidString("exit"))

    pendingIntent = PendingIntent.getActivity(
        context, 321321, i, PendingIntent.FLAG_UPDATE_CURRENT)
    title = cast(
        'java.lang.CharSequence', AndroidString("Thoát ứng dụng ?")
    )

    action = ActionBuilder(
        context.getApplicationInfo().icon, title, pendingIntent)
...
def notify(
    ...
    if channel_id == 'RQSRD_ALARM':
        ID = 123123
    elif channel_id == 'RQSRD_ALARM_HEADS_UP':
        ID = 121212
        action = create_exit_intent(context)
    ...
    notificationManager.notify(10101010, notification)
